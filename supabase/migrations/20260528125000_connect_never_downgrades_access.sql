-- ============================================================================
-- Фикс: client_connect_to_lawyer НЕ должен закрывать уже открытый доступ.
--
-- Симптом: клиент открыл доступ тумблером (grant_access=true → is_active=true),
-- затем нажал «Написать в чат» (client_connect_to_lawyer с grant_access=false).
-- Прежний апсерт делал DO UPDATE SET is_active = p_grant_access → затирал true
-- обратно в false, и в чате снова появлялся баннер «откройте доступ».
--
-- Решение: апсерт только ПОВЫШАЕТ доступ (is_active OR EXCLUDED.is_active).
-- Понизить доступ можно только явно — через client_revoke_lawyer_access.
-- Остальная логика (проверка clients_limit, реанимация карточки) — как в
-- 20260528123000.
-- ============================================================================
...
BEGIN
  ...
  -- Доступ только ПОВЫШАЕМ: уже открытый доступ нельзя случайно закрыть
  -- повторным подключением с grant_access=false (это делает только revoke).
