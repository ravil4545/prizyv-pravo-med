...
6. ШАНС НЕПРИЗЫВНОЙ КАТЕГОРИИ (В):
   КРИТИЧЕСКИ ВАЖНЫЕ ПРАВИЛА РАСЧЁТА:
   
   МИНИМАЛЬНЫЕ ПОРОГИ (обязательны!):
   - АНАЛИЗ с подтверждённым диагнозом (кровь, моча, биохимия и т.д.): МИНИМУМ 50%
   - ОБСЛЕДОВАНИЕ с подтверждённым диагнозом (рентген, МРТ, КТ, УЗИ, ЭКГ): МИНИМУМ 75%
   - Заключение узкого специалиста с диагнозом: МИНИМУМ 60%
   - Выписка из стационара: МИНИМУМ 70%
   
   ТИПЫ ДОКУМЕНТОВ И БАЗОВЫЙ ШАНС:
   - Рентген/МРТ/КТ с патологией: 75-90% (это ОБСЛЕДОВАНИЕ)
   - УЗИ с патологией: 70-85% (это ОБСЛЕДОВАНИЕ)
   - ЭКГ/ЭхоКГ с патологией: 70-85% (это ОБСЛЕДОВАНИЕ)
   - Анализ крови/мочи с отклонениями: 50-70% (это АНАЛИЗ)
   - Биопсия с патологией: 80-95% (это ОБСЛЕДОВАНИЕ)
   - Заключение специалиста: 60-80%
   - Амбулаторная карта с лечением: 55-75%
   
   ПОВЫШАЮЩИЕ ФАКТОРЫ (+5-10% каждый):
   - Хроничность заболевания (более 1 года)
   - Наличие деформаций/патологий на снимках
   - Регулярное лечение
   ...
   - Рентген стоп с плоскостопием: 75-85% (обследование с патологией)
   - МРТ позвоночника с грыжей: 80-90%
   - УЗИ почек с камнями: 75-85%
   - Анализ крови с анемией: 50-65%
   - ЭКГ с аритмией: 75-85%
   
   ТОЛЬКО 0-30%: документы БЕЗ патологий, анализы в норме, справки о здоровье
...
Верни результат СТРОГО в формате JSON:
{
  ...
  "categoryBChance": число от 0 до 100 (минимум 50 для анализов с диагнозом, минимум 75 для обследований с патологией!),
