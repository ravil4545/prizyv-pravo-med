...
const normalizeAsthmaAnalysisResult = (
  ...
  const asthmaExplanation =
    ...
    (hasDebut
      ? " Пометка «дебют» означает впервые выявленное заболевание, а не отсутствие диагноза; без истории наблюдения, госпитализаций или объективных ФВД документальные подтверждения пока умеренные. Это не прогноз категории и не вероятность решения комиссии."
      : "");
...
- Для дебюта заболевания, единственного частного заключения и отсутствия объективной функциональной проверки снижай оценку силы подтверждений, не отрицая сам документированный диагноз.
...
3. КАТЕГОРИЯ И СИЛА ДОКУМЕНТАЛЬНЫХ ПОДТВЕРЖДЕНИЙ:
   Дай предварительную категорию и оцени силу подтверждений только по данным текста. Не применяй универсальное правило «хроническое заболевание = В» и не выдавай оценку за вероятность исхода.
   При неполном подтверждении снижай оценку силы подтверждений и кратко объясняй недостающий квалифицирующий факт.
...
6. СИЛА ДОКУМЕНТАЛЬНЫХ ПОДТВЕРЖДЕНИЙ:
   Оцени силу именно этого документа: полноту диагноза, объективные критерии, источник, даты и непротиворечивость.
   При единственном заключении, дебюте, подозрении или отсутствии квалифицирующего показателя снижай оценку силы подтверждений и объясняй причину. Не выдавай её за вероятность исхода.
