...
// Экспортируются: конструктор документа (lib/documentBlocks.ts) собирает из них
// те же шапку/подпись/отметку, что и готовые шаблоны. Копия вместо импорта
// неминуемо разошлась бы с каталогом при первой же правке реквизитов.
export const FROM_MIL = `от {{full_name}}
{{birth_date}}, {{birth_place}}
адрес для ответа: {{registration_address}}
тел.: {{phone}}, e-mail: {{email}}
воинский учёт: {{military_commissariat}}`;

export const FROM_MED = `от пациента: {{full_name}}
...
export const SIGN = `{{today}}                    _______________ / {{full_name}} /`;

export const ACCEPT = `Отметка о принятии (заполняется органом при подаче):
...
export const APPENDIX = `{{#if docs_list}}Приложения:
{{docs_list}}{{/if}}`;
